...
try {
	...
	const tableExists = await tableManager.tableExists("test-users");
	if (!tableExists) {
	...
	const emailQuery = User.query({ email: "test@example.com" }).usingIndex(
		"EmailIndex",
	);
	...
	console.log(
		"✅ Complex GSI query compiles successfully:",
		typeof statusQuery,
	);
	...
	try {
		...
		User.query({ invalidField: "value" }).usingIndex("EmailIndex");
	} catch (error: any) {
	...
	// clean up
	await tableManager.deleteTable("test-users");
} catch (error) {
