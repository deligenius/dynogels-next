...
Table.prototype.sendRequest = function (method, params, callback) {
  ...
  driver[method].call(driver, params, (err, data) => {
    ...
    if (err) {
      this.log.warn({ err: err }, 'dynogels %s error', method.toUpperCase());
      return callback(err);
    } else {
      this.log.info({ data: data }, 'dynogels %s response - %sms', method.toUpperCase(), elapsed);
...
Table.prototype.get = function (hashKey, rangeKey, options, callback) {
  ...
  this.sendRequest('get', params, (err, data) => {
    if (err) {
      return callback(err);
    }
    ...
    let item = null;
    if (data.Item) {
      item = this.initItem(this.serializer.deserializeItem(data.Item));
    }
...
Table.prototype.create = function (item, options, callback) {
  ...
  if (_.isArray(item)) {
    async.map(item, (data, callback) => internals.createItem(this, data, options, callback), callback);
  } else {
    return internals.createItem(this, item, options, callback);
  }
};
...
Table.prototype.update = function (item, options, callback) {
  ...
  if (schemaValidation.error) {
    return callback(_.assign(new Error(`Schema validation error while updating item in table ${this.tableName()}: ${JSON.stringify(schemaValidation.error)}`), {
  ...
  internals.callBeforeHooks(this, 'update', start, (err, data) => {
    if (err) {
      return callback(err);
    }
    ...
    let exp = null;
    try {
      exp = internals.updateExpressions(this.schema, data, options);
    } catch (e) {
      return callback(e);
    }
    ...
    this.sendRequest('update', params, (err, data) => {
      if (err) {
        return callback(err);
      }
      ...
      let result = null;
      if (data.Attributes) {
        result = this.initItem(this.serializer.deserializeItem(data.Attributes));
      }
...
Table.prototype.destroy = function (hashKey, rangeKey, options, callback) {
  ...
  this.sendRequest('delete', params, (err, data) => {
    if (err) {
      return callback(err);
    }
    ...
    let item = null;
    if (data.Attributes) {
      item = this.initItem(this.serializer.deserializeItem(data.Attributes));
    }
...
Table.prototype.runQuery = function (params, callback) {
  ...
  this.sendRequest('query', params, internals.deserializeItems(this, callback));
};

Table.prototype.runScan = function (params, callback) {

  this.sendRequest('scan', params, internals.deserializeItems(this, callback));
};

Table.prototype.runBatchGetItems = function (params, callback) {
  this.sendRequest('batchGet', params, callback);
};
...
Table.prototype.createTable = function (options, callback) {
  ...
  const params = this.dynamoCreateTableParams(options);

  this.sendRequest('createTable', params, callback);
};
...
Table.prototype.updateTable = function (throughput, callback) {
  ...
  async.parallel([
    async.apply(internals.syncIndexes, this),
    async.apply(internals.updateTableCapacity, this, throughput),
  ], callback);
};
